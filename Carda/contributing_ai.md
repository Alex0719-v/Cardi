...
- 名片信息栏中手机、邮箱、地址图标来源为 `My icon/Phone.svg`、`My icon/Mail.svg`、`My icon/Map pin.svg`；不得继续使用旧的 SwiftUI 手绘版本，也不得用 `WKWebView` / HTML 异步加载本地图标。
- 底部导航和名片信息栏这类高频重建区域中的本地 SVG 图标必须复用 `LocalSVGIconView` 的缓存解析和 SwiftUI `Canvas` 渲染路径，避免页面切换或名片滑动时出现闪烁、延迟。
