...
本轮已修复：不再把 `supportsDirectionMeasurement == false` 误判为整机不支持；未锁定目标时不再冻结上划副本；发送事务加入 12 秒 ACK 超时；未连接 send、对端 `.error`、断线与超时统一回收当前意图和事务。
...
两台支持 precise distance 的 iPhone 安装同一 Release/TestFlight 构建；至少覆盖一组支持 direction 与一组 direction 不可用的组合。测试前在两机 `设置 → 帮助与关于` 连续轻点 `当前版本` 7 次打开 `交换诊断`，填写相同六位编号并分别选择设备 A/B 后开始记录。每个测试必须同时保存：
...
| 隐藏交换诊断入口 UI 自动化 | PASS | 版本行 7 次解锁、六位编号、开始/结束记录、导出状态 |
