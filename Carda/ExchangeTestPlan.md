...
Cardi 设计的“双方前台打开 App、停留在我的名片页、通过上划表达发送意图、用 Nearby Interaction 选择 1.5 米内方向明确对象、再通过点对点连接传输名片”的方案，在 Apple 公布的系统能力边界内**原则上可实现**。
...
2. 发送事务没有 ACK / rejected / send failure 的统一超时回收；丢包或静默发送失败后可能永久阻止下一次上划。
3. payload 校验上限与接收消息上限矛盾：校验允许的最大 payload 编码后为 16,845,575 字节，超过 14,680,064 字节的接收上限。
4. UI 只保存一个 `queuedReceivedCard`，多人同时投递时后来的来卡可能覆盖前一条未展示事务。
5. Nearby Interaction 权限拒绝、Local Network 权限拒绝和系统无线错误没有被分成可恢复、可引导的明确状态。
6. Multipeer Connectivity 在当前 iOS 26.5 SDK 中已弃用并要求迁移到 Network framework；它仍可编译，但属于上线后的兼容性与维护风险。
...
- 目标必须在 1.5 米内、手机前方约 ±45°、稳定至少 0.4 秒。
...
- `NISession.deviceCapabilities.supportsPreciseDistanceMeasurement` 与 `supportsDirectionMeasurement` 可用于运行时能力判断。
...
两台支持 precise distance 与 direction 的 iPhone 安装同一 Release/TestFlight 构建。每个测试必须同时保存：

- 两机型号、系统版本、构建号、权限状态和网络状态。
- 两机屏幕录制，画面中可识别时间基准。
- CardExchange / NearbyExchangeRanging 的 OSLog。
...
2. 一台支持、一台不支持 direction 的组合，预期禁止自动投递并给出明确说明。
...
- 当前 deployment target 为 iOS 26.5；若要覆盖更广用户，必须先确认并下调最低版本，再补对应 OS 矩阵。
...
- B03 `DEVICE` 支持距离但不支持 direction 时不得按距离单独发送。
...
- F05 `AUTO` 零向量、NaN、infinite 和缺失 direction 不可选。
...
- G05 `DEVICE` B 点击“分到列表”：保存到正确默认列表后 ACK。
...
2. **发送事务无最终超时。** `outgoingTransactions` 非空会阻止新手势，但没有 ACK/rejected 截止时间；连接仍在却 ACK 丢失时可能永久卡住。
3. **静默 send 丢弃。** transport 在 peer 不 connected 时直接 return，coordinator 已登记 outgoing 并进入等待，可能没有任何回调清理。
4. **payload / transport 上限冲突。** 最大校验通过 payload 超过接收消息上限，会被接收端 guard 静默丢弃。
5. **无效 payload 回复类型与事务清理不一致。** 接收端发送 `.error`，发送端只更新文案，不移除对应 outgoing transaction。
6. **NI 权限拒绝未识别。** `userDidNotAllow` 需要停止重建并明确引导用户去 Settings；当前 invalidation 路径会尝试重建与重新 run。
