...
2. payload 校验上限与接收消息上限矛盾：校验允许的最大 payload 编码后为 16,845,608 字节，超过 14,680,064 字节的接收上限。
...
本轮已修复：不再把 `supportsDirectionMeasurement == false` 误判为整机不支持；未锁定目标时不再冻结上划副本；发送事务加入 12 秒 ACK 超时；未连接 send、对端 `.error`、断线与超时统一回收当前意图和事务。2026-07-23 测试编号 `050212` 的首轮双机 JSON 暴露诊断开始后仍停留在账户 Sheet 的问题，现已改为自动关闭 Sheet、退出搜索并回到“我的名片”。2026-07-24 第二轮同编号 A/B JSON 中，两端都已经记录 `diagnostic_return_to_my_cards_requested`、`advertising_started` 与 `exchange_coordinator_started`，且能力均为 precise distance=true、direction=false；但两端都没有 `gesture_primed`、`browsing_started` 或 peer 事件，说明失败仍在 discovery 之前，具体是整页祖先层的并列分页/递卡手势没有把真机上划传给递卡回调。现已把纵向递卡手势迁移到当前名片稳定命中面并统一到画板命名坐标系，同时新增真实屏幕上划 UI 回归。
...
两台支持 precise distance 的 iPhone 安装同一 Release/TestFlight 构建；至少覆盖一组支持 direction 与一组 direction 不可用的组合。测试前在两机直接进入常驻的 `设置 → 帮助与关于 → 交换诊断`，填写相同六位编号并分别选择设备 A/B 后开始记录。两机都应自动关闭账户 Sheet 并回到“我的名片”；开始实际上划前，状态页或导出 JSON 必须已经出现 `advertising_started` 与 `exchange_coordinator_started`，否则本轮不算进入交换测试。每个测试必须同时保存：
...
- D11 `SIM/DEVICE` 当前名片稳定命中面必须收到真实屏幕上划；不得因整页分页、点击或长按组合而缺失 `gesture_primed` / `browsing_started`。Simulator 自动化至少断言进入 NI 能力检查、方向发现或 distance-only 最近目标确认状态之一。
...
| 新增交换 UI 自动化 | PASS 4/4 | `CardExchangeSimulationUITests`，含真实屏幕上划入口，0 failures |
| 常驻交换诊断入口 UI 自动化 | PASS | 无需解锁即可进入、六位编号、开始后自动返回“我的名片”、重新进入后结束记录、导出状态 |
| 交换诊断隐私 standalone 测试 | PASS | 六位编号规范化、peer 摘要稳定且 JSON 不含明文 peer ID |
| 目标选择边界 standalone 测试 | PASS | 距离/角度/稳定/TTL/歧义/最近目标 |
| payload 限制 standalone 测试 | PASS + KNOWN RISK | 最大合法编码 16,845,608 > 14,680,064 |
...
6. **iOS 17/18 真机运行回归尚未完成。** deployment target 已是 iOS 17.0，低于 iOS 26 的 Liquid Glass 也有降级实现，但当前交换证据主要来自 iOS 26 与 Simulator；仍需补齐 iOS 17/18 真机运行、权限和交换矩阵。
